 ...
 * Check-in.
 *
 * This screen lives at the ROOT of the router, not inside the Məşq tab, because
 * two different tabs open it. The Kəşf gym page used to push
 * `/(tabs)/workout/checkin`, which switched the focused tab to Məşq; the
 * `router.back()` below then popped inside the MƏŞQ stack, so somebody who
 * started on a gym page in Kəşf was returned to Məşq home and had to find their
 * way back to the gym by hand. From the root stack, back is back — to whichever
 * tab and screen the person actually came from.
 *
 * `src/app/(tabs)/workout/checkin.tsx` re-exports this so the Məşq tab's own
 * «Check-in» tile keeps its in-tab route, where back is also correct.
 */
...
export default function CheckIn() {
  ...
  /* The SAME component is mounted at two paths (see the file header). Inside the
     Məşq tab the scene is already padded by the native tab bar's reserved space,
     home indicator folded in; at the root there is no tab bar under it and
     nothing reserves that strip, so the «Check-in et» button would sit on top of
     the gesture bar. One inset, added only where it is actually missing. */
  const segments = useSegments();
  /* Read once, on mount. `useSegments` reports the FOCUSED route, so the moment
     this screen starts sliding away the answer flips — and the footer would jump
     34 pt while the «Check-in edildi» card is still on screen closing itself. */
  const [inTabScene] = useState(() => String(segments[0]) === '(tabs)');
  const footerPad = inTabScene ? 10 : insets.bottom + 10;
  ...
  /* The «check-in edildi» screen closes itself after 1.4 s. The handle was
     dropped on the floor, so leaving first — the × in the header, or the
     hardware back — did not stop it: it fired anyway and popped whatever screen
     the person had opened in the meantime. Tap ×, start today's workout, and the
     session screen vanished a second later with nothing to blame it on. */
  ...
  const checkIn = () => {
    ...
    gate(async () => {
      ...
      setSynced(true);
      successFeedback();
      closeRef.current = setTimeout(() => router.back(), 1400);
    }, 'Check-in üçün');
  };
      ...
      {!gymId ? (
        ...
        <>
          ...
          <View style={[styles.footer, { paddingBottom: footerPad }]}>
            {/* `navigate`, not `replace`: from the root stack `replace` would swap
                this screen for a SECOND (tabs) navigator stacked on top of the one
                already underneath it. `navigate` pops back to the tabs that are
                there and focuses Kəşf, and inside the Məşq tab it still just
                switches tab — one call that is correct at both paths. */}
            <PressableScale onPress={() => router.navigate('/(tabs)/discover')} style={styles.primaryBtn}>
        ...
        <>
                ...
                {!done
                  ? refused
                    ...
                    : verifying
                      ...
                      : 'Check-in zalın 150 m radiusunda və iş saatlarında edilir. Seriyanı davam etdirir, kimin zalda olduğunu göstərir və rəy yazmaq hüququ verir.'
                  : synced === null
                    ? `${gymLabel} · yoxlanılır…`
                    : synced
                      ? `${gymLabel} · seriya ${stats.streakDays} gün. Zala gələnlər səni indi görür.`
                      : `${gymLabel} · seriya ${stats.streakDays} gün. Check-in yalnız bu cihazda qeyd olundu — serverə çatmadı, başqaları səni indi zalda görmür.`}
              </AppText>
              {!done && !refused && gym ? (
          ...
          <View style={[styles.footer, { paddingBottom: footerPad }]}>
            <View style={styles.streakCard}>
              ...
              {/* «Seriya», not «streak». The same counter was called by an
                  untranslated English word here and «Seriya» on Profil and
                  Analitika — one number with two names, one tap apart, in an
                  app whose UI is Azerbaijani. */}
              <AppText style={{ flex: 1, color: 'rgba(255,255,255,0.8)', fontSize: 13.5, fontWeight: '500' }}>
                {checkedInToday
                  ? `Seriya: ${stats.streakDays} gün · bugün artıq check-in etmisən`
                  : stats.streakDays === 0
                    ? 'İlk check-in — seriya bugün başlayır'
                    : `${stats.streakDays} gün seriya · bugün check-in etsən ${stats.streakDays + 1} olacaq`}
              </AppText>
            </View>
            <PressableScale
              onPress={checkIn}
...
const styles = StyleSheet.create({
  ...
  /* No bottom padding of its own — `footerPad` above supplies it, because the
     right value depends on which of the two paths this screen was opened at.
     Inside the Məşq tab scene (tabs)/_layout already reserves the native tab
     bar's space with the home indicator folded in, and the footer used to add
     `insets.bottom` again on top of that. */
